  ...
  // ── OPENING CHECKS ──────────────────────────────────────
  {
    ...
    items: [
      ...
      },
      {
        ...
        type: 'check',
        label: 'LUCCA A53 — purged, brew temp 200°F, steam pressure ~1.2 bar',
        labelEs: 'LUCCA A53 — purgada, temperatura de extracción 200°F, presión vapor ~1.2 bar',
        note: 'Group head should be hot to the touch. Run two blank flushes before the first shot.',
        noteEs: 'El cabezal debe estar caliente al tacto. Realiza dos descargas en blanco antes del primer shot.',
      },
      {
        id: 'lucca_daily_backflush',
        type: 'check',
        label: 'LUCCA A53 — daily plain-water backflush (no chemical)',
        labelEs: 'LUCCA A53 — retrolavado diario con agua pura (sin químico)',
        note: 'Blind basket + 30 sec on, 5 sec off × 5 cycles. Chemical backflush is WEEKLY only — see periodic checklist.',
        noteEs: 'Filtro ciego + 30 seg encendido, 5 seg apagado × 5 ciclos. Retrolavado químico es SEMANAL — ver lista periódica.',
      },
      {
        id: 'grinders',
        type: 'check',
        label: 'Grinders — burrs clean, dose calibrated (18g target), hopper level checked',
        labelEs: 'Molinos — muelas limpias, dosis calibrada (objetivo 18g), nivel de tolva verificado',
        note: 'Pull two test shots and dial in (target 36g out in 25–30s). Recalibrate if humidity/temperature changed overnight.',
        noteEs: 'Realiza dos shots de prueba y ajusta (objetivo 36g salida en 25–30s). Recalibra si cambió humedad/temperatura durante la noche.',
      },
      {
        id: 'pos_system',
        type: 'check',
...
export const CLOSING_ITEMS = [
  // ── CLOSING CHECKS ───────────────────────────────────────
  {
    ...
    items: [
      ...
      {
        id: 'square_eod_report_pulled',
        type: 'check',
        label: 'Square — End-of-Day report pulled (Reports → Sales summary → Today)',
        labelEs: 'Square — informe Fin del Día generado (Informes → Resumen de Ventas → Hoy)',
        note: 'Record the gross sales total + tip total before counting drawer.',
        noteEs: 'Registra ventas brutas + propinas totales antes de contar la caja.',
      },
      {
        id: 'square_eod_total',
        type: 'text',
        label: 'Square End-of-Day total ($)',
        labelEs: 'Total Fin del Día de Square ($)',
        note: '',
        noteEs: '',
        required: true,
      },
      {
        id: 'drawer_counted',
        type: 'check',
        label: 'Drawer counted — currency + coin total recorded',
        labelEs: 'Caja contada — total de billetes + monedas registrado',
        note: 'Float remains in drawer ($150 default). Count only the day\'s take.',
        noteEs: 'El fondo permanece en la caja ($150 por defecto). Cuenta solo lo del día.',
      },
      {
        id: 'drawer_count_total',
        type: 'text',
        label: 'Drawer count (cash sales only, $)',
        labelEs: 'Conteo de caja (solo ventas en efectivo, $)',
        note: '',
        noteEs: '',
        required: true,
      },
      {
        id: 'variance_check',
        type: 'check',
        label: 'Variance vs Square cash total is within ±$5 (flag if not)',
        labelEs: 'Variación vs total en efectivo de Square dentro de ±$5 (marcar si no)',
        note: 'Variance > $5 = flagged item. Document cause: miscount, comp, refund, suspected theft.',
        noteEs: 'Variación > $5 = elemento marcado. Documenta causa: error de conteo, cortesía, reembolso, sospecha de robo.',
      },
      {
        id: 'deposit_prepared',
        type: 'check',
        label: 'Cash deposit prepared (envelope sealed, dated, signed by closer)',
        labelEs: 'Depósito preparado (sobre sellado, fechado, firmado por quien cierra)',
        note: 'Deposited next business day. Owner holds the bag overnight in locked safe.',
        noteEs: 'Depositado el siguiente día hábil. Propietario guarda el sobre en caja fuerte.',
      },
      {
        id: 'unit_locked',
        type: 'check',
